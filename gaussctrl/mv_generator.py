...
from gaussctrl.mv_model import MVGaussctrlModel
import cv2
...
class DepthGenerator(pl.LightningModule):
    def __init__(self, config):
        ...
        self.mv_base_model = MVGaussctrlModel(
            unet, config['model'])
    ...
    def gen_cls_free_guide_pair(self, latents, timestep, prompt_embd, batch, type='generation'):
        ...
        mats_view=torch.cat([batch['mats_view']]*2)
        mats_proj=torch.cat([batch['mats_proj']]*2)
        depths=torch.cat([batch['depths']]*2)
        meta['mats_view']=mats_view
        meta['mats_proj']=mats_proj
        meta['depths']=depths
    ...
    def inference_inp(self, batch):
        ...
        latents = torch.randn(
            bs, 1, 4, h//8, w//8, device=device).repeat(1,m,1,1)
    ...
    def inference_gen(self, batch):
        ...
        for i, t in enumerate(timesteps):
            ...
            latents = self.scheduler.step(
                noise_pred, t, latents).prev_sample
        print(f"denoise done. starting decoding")
        images_pred = self.decode_latent(latents)
        return images_pred

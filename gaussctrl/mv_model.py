...
from gaussctrl.mv_modules import CPBlock, ImageEncodingBlock
from einops import rearrange
import numpy as np 

from gaussctrl.mv_depth_utils import *


class MVGaussctrlModel(nn.Module):
    ...
    def get_correspondence(self, cp_package):
        ...
        for b_i in range(b): # per batch
            for i in range(m): # per frame
                for j in range(i+1,m): # per remaining frame
